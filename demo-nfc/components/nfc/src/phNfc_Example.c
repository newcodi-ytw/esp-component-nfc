...
void phNfc_Example_Init(void)
{
    ...
    DEBUG_PRINTF("DiscoveryLoop Example:");
#ifdef PH_PLATFORM_HAS_ICFRONTEND
...
void phNfc_Example_Main(void  *pDataParams)
{
    ...
    while (1)
    {
        DEBUG_PRINTF("Detect LOOP: %d", cnt++);
        ...
        else
        {
            ...
            statustmp = phhalHw_Wait(pHal, PHHAL_HW_TIME_MICROSECONDS, 1000);
            CHECK_STATUS(statustmp);
        }
    }
...
uint16_t NFCForumProcess(uint16_t wEntryPoint, phStatus_t DiscLoopStatus)
{
    ...
    DEBUG_PRINTF("wEntryPoint: %d DiscLoopStatus: %d", wEntryPoint, DiscLoopStatus);
    if(wEntryPoint == PHAC_DISCLOOP_ENTRY_POINT_POLL)
    {
        ...
        {
            DEBUG_PRINTF("PHAC_DISCLOOP_NO_TECH_DETECTED");
            DEBUG_PRINTF("PHAC_DISCLOOP_NO_DEVICE_RESOLVED");
            DEBUG_PRINTF("Switch to LISTEN mode after POLL mode");
            /* Switch to LISTEN mode after POLL mode */
        }
        else if((DiscLoopStatus & PH_ERR_MASK) == PHAC_DISCLOOP_EXTERNAL_RFON)
        {
            DEBUG_PRINTF("can restart the loop in LISTEN mode");
            /*
        ...
        DEBUG_PRINTF("pdate the Entry point to LISTEN mode");
    }
    else
    {
        if((DiscLoopStatus & PH_ERR_MASK) == PHAC_DISCLOOP_EXTERNAL_RFOFF)
        {
            DEBUG_PRINTF(">>PHAC_DISCLOOP_EXTERNAL_RFOFF<<");
            /*
            ...
            DEBUG_PRINTF("LISTEN_PHASE_TIME_MS: %d", LISTEN_PHASE_TIME_MS);
            status = phhalHw_EventWait(pHal, LISTEN_PHASE_TIME_MS);
            DEBUG_PRINTF("phhalHw_EventWait: %d", status);
