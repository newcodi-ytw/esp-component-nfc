 ...
 * This function will set the parameter required to behave as target for Type A and Type F.
 *
 * \return Status code
 * \retval #PH_ERR_SUCCESS Operation successful.
...
#define  phhalHw_EventConsume(pDataParams)  \
    ...
    *   The buffer may contain the result of one or more time slots. Each time slot result has the following structure:
    ...
    *        2 ... No tag replied in time slot. 'Tag reply length field', 'Valid bits in last byte field' and 'Tag reply field', 'Tag Handle' are not present.
    *        3 ... Two or more tags responded in the time slot. (Collision). Tag Reply Length, valid bits in last byte and tag reply field not present.
    *       )
