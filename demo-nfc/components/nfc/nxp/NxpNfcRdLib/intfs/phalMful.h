...
#define phalMful_ReadSign(pDataParams, bAddr, pSignature) \
        phalMful_Sw_ReadSign((phalMful_Sw_DataParams_t *)pDataParams, bAddr, pSignature, &wDataLen)
...
#define phalMful_ReadTTStatus(pDataParams, bAddr, pData) \
        phalMful_Sw_ReadTTStatus((phalMful_Sw_DataParams_t *)pDataParams, bAddr, pData)
...
/** \name Diversification option for Ultralight EV1 Authenticate command. */
/** @{ */
#define PHAL_MFUL_CMD_PWD_AUTH_DIV_OFF                      0x00    /**< Option to indicate the diversification is disabled. */
#define PHAL_MFUL_CMD_PWD_AUTH_DIV_ON                       0x02    /**< Option to indicate the diversification is enabled and the diversification should be performed
                                                                     *   with DivInput. In this cases the Div algorithm will be CMAC(Kx, Const | M | Padding), where
                                                                     *   M is diversification input (1 - 31 bytes)
                                                                     */
/**
 * end of Diversification option
 * @}
 */
...
phStatus_t phalMful_PwdAuth(
        ...
        uint8_t bOption,                                            /**< [In] One of the below options should be used. This option byte is supported for Sam component only.
                                                                     *          \arg PHAL_MFUL_CMD_PWD_AUTH_DIV_OFF
                                                                     *          \arg PHAL_MFUL_CMD_PWD_AUTH_DIV_OFF_INPUT_NOT_AVAILABLE
                                                                     *          \arg PHAL_MFUL_CMD_PWD_AUTH_DIV_ON
                                                                     */
        ...
        uint8_t * pPwd,                                             /**< [In] Four byte array of type uint8_t containing four password bytes. \n
                                                                     *          \c For software layer, the user has to pass the password information.\n
                                                                     *          \c For Sam AV3 NonX layer, the parameter will be updated with the password generated
                                                                     *             by Sam hardware.
                                                                     */
        uint8_t * pPack                                             /**< [Out] Two byte array of type uint8_t containing two password acknowledge bytes \n
                                                                     *          \c For software and Sam AV3 NonX layer, the user will be provided with the Pack \n
