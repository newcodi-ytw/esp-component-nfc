    ...
    uint8_t ** ppResponse, uint16_t * pRxlen)
{
    ...
    if (0U != (bWrappedMode))
    {
        ...
        pApdu[4] = (uint8_t)((wCmdLen - (uint8_t)1U) & 0xFF);

        PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
            pPalMifareDataParams,
            (uint16_t)(pApdu[4] == 0x00U) ? PH_EXCHANGE_DEFAULT : PH_EXCHANGE_BUFFER_FIRST,
            pApdu,
        ...
        if (pApdu[4] != 0x00U)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp,  phpalMifare_ExchangeL4(
                ...
                &pSendBuff[1],
                (((wCmdLen - 1) < 0) ? 0 : ((wCmdLen - 1) & 0xFFFF)),
                &pRecv,
                pRxlen));
    ...
    uint8_t bLc, uint8_t bExtendedLenApdu, uint8_t * pDataIn, uint8_t bLe, uint8_t ** ppDataOut, uint16_t *pDataLen)
{
    ...
    if (0U != (bOption & 0x01U))
    {
        ...
        if(bExtendedLenApdu == 0x01U)
        {
            ...
            wCmdLen = (uint16_t)((wCmdLen + 1U) & 0xFFFF);
