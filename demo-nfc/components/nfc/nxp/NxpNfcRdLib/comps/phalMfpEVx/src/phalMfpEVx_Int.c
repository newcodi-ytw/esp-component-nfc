    ...
    if(0U != bLayer4Comm)
    {
        ...
        if(0U != bWrappedMode)
        {
            ...
            PH_CHECK_SUCCESS_FCT(wStatus, phalMfpEVx_Int_Send7816Apdu(
                pPalMifareDataParams,
                PH_EXCHANGE_DEFAULT,
                (uint16_t)((bCmdLen - 1U) & 0xFFFF),   /* Excluding the command code */
                bExtendedLenApdu,
                aCmdBuff,
    ...
    if((wOptions == PH_EXCHANGE_BUFFER_FIRST) || (wOptions == PH_EXCHANGE_DEFAULT))
    {
        ...
        if(0U != wLc)
        {
            ...
            PH_CHECK_SUCCESS_FCT(wStatus, phpalMifare_ExchangeL4(
                ...
                (uint16_t)(((wDataLen - 1) < 0)? 0 : ((wDataLen - 1) & 0xFFFF)), /* Subtracted - 1 because command code is not available as part of data. */
                NULL,
                NULL));
        }
        else
