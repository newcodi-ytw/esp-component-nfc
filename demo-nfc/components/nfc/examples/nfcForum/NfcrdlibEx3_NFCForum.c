...
int NfcrdlibEx3_NFCForum_Main(void)
{
    do
    {
...
void TReaderLibrary(void  *pDataParams)
{
    ...
    while(bInfLoop)
    {
        ...
        status = phhalHw_Wait(pHal, PHHAL_HW_TIME_MICROSECONDS, 51000);
        CHECK_STATUS(status);
...
phStatus_t ReadNdefMessage(uint8_t TopTagType)
{
    ...
    if((bTagState == PHAL_TOP_STATE_READONLY) || (bTagState == PHAL_TOP_STATE_READWRITE))
    {
        ...
        if(dwDataLength)
        {
            ...
            DumpBuffer(baSnepAppBuffer, dwDataLength>500?500:dwDataLength);
            // DumpBuffer(baSnepAppBuffer, 50);
            // PCD_HelpShowByte("NDEF:", baSnepAppBuffer, dwDataLength);
            // ESP_LOG_BUFFER_HEXDUMP("NDEF:", baSnepAppBuffer, dwDataLength, ESP_LOG_INFO);
        }
        else
        {
